...
public class LinkMatrix {
    ...
    // similarity above the threshold param
    ...
    /**
    @params  points: DataPoint[], 特征点
    @param   pointSim: SimilarityMeasure, 计算相似度的实现
    @param   th: Double, threshold
    */
    ...
    private void init(DataPoint[] points, double[][] similarityMatrix, double th) {
        ...
        for(int i = 0; i < n; i++) {
            for(int j = i + 1; j < n; j++) {
                if( pointSimilarityMatrix[i][j] >= th ) { // greeter than threshold
    ...
    // todo: what's these means?
    private int nLinksBetweenPoints(int[][] neighbors, int indexX, int indexY) {
        int nLinks = 0;
        for(int i = 0, n = neighbors.length; i < n; i++) {// [indexX, indexY] 十字形的矩阵所有值得和
            nLinks += neighbors[indexX][i] * neighbors[i][indexY]; // 注意这个地方的 * 
        }
        return nLinks;
    }
     ...
     */
    private double[][] calculatePointSimilarities(DataPoint[] points, SimilarityMeasure pointSim) {
