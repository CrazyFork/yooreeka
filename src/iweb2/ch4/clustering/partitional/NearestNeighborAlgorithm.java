...
public class NearestNeighborAlgorithm {
    ...
    private double getNNDistance(Cluster c, DataPoint x) {
        ...
        if( c.contains(x) ) { // inside this cluster
            nnDist = 0.0;
        }
        else {
            ...
            for(DataPoint y : c.getElements()) {
                ...
                double xyDist = a[i][j];
                nnDist = Math.min(nnDist, xyDist); // take the nearest.
            }
        }
    ...
    private void assignPointToCluster(DataPoint x) {
        ...
        for(Cluster c : allClusters) {
            double nnDist = getNNDistance(c, x); // check this point agaist all the exsiting cluster
            if( nnDist < minNNDist ) {
                minNNDist = nnDist;
                closestCluster = c;
            }
    ...
    public static void main(String[] args) {
        ...
        NearestNeighborAlgorithm nn = new NearestNeighborAlgorithm(elements, a, threshold);
